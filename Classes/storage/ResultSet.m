...
+ (id) resultSetWithStatement:(Statement *)statement usingParentDatabase:(Database*)aDB {
...
- (BOOL) next {
	int result = [parentDB executeStatement:[statement statement]];
...
- (void)setParentDB:(Database *)newDb {
	parentDB = newDb;
}
...
- (int) dataTypeForColumnIndex:(int)columnIdx
{
    return sqlite3_column_type(statement.statement, columnIdx);
}

- (int) dataTypeForColumn:(NSString*)columnName
{
    return [self dataTypeForColumnIndex:[self columnIndexForName:columnName]];
}
